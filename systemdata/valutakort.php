...
echo "$difkto && is_numeric($kodenr) && $dato && $kurs && $dato!=\"-\" && $kurs!=\"-\"<br>";
...
#echo $saldo[$x]*$kurs/100."<br>";
#echo $saldo[$x]*$gl_kurs/100."<br>";
#echo "$diff=afrund($saldo[$x]*$kurs-$saldo[$x]*$gl_kurs,3)<br>";
#				$diff=afrund($saldo[$x]*$kurs/100-$saldo[$x]*$gl_kurs/100,3);
				...
				echo "$qtxt<br>";
				if ($diff) db_modify($qtxt,__FILE__ . " linje " . __LINE__);
				echo "update kontoplan set valutakurs='$ny_kurs' where kontonr='$kontonr[$x]' and regnskabsaar='$regnaar'<br>";
				...
				for ($y=0;$y<count($debkredgrp);$y++){
					...
					for ($z=0;$z<count($adr_konto_id);$z++){
						$dkksum[$z]=0;
						echo "select amount,valutakurs from openpost where udlignet='0' and konto_id='$adr_konto_id[$z]'<br>";
